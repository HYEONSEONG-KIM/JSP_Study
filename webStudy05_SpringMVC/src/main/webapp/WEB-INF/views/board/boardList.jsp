...
<script type="text/javascript">
 	...
 	let paging = $('#pagingArea');
 	
 	function makeTds(value){

		return $('<tr>').append(
			$("<td>").html(value.rnum),	
			$("<td>").html(value.boTitle),	
			$("<td>").html(value.boWriter),	
			$("<td>").html(value.boDate),	
			$("<td>").html(value.boHit),	
			$("<td>").html(value.boRep),	
			$("<td>").html(value.boRec)
			).attr("id",value.boNo);
		...
		let searchForm = $("#searchForm").paging()
					.ajaxForm({
						...
						success : function(resp){
							...
							let trs = [];
							$(boardList).each(function(idx, value){
								trs.push(makeTds(value));
							});
							tbody.append(trs);
							paging.append(resp.pagingHTML);
